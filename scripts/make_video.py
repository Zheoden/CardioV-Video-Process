...
def make_video_from_edge_maps(img_list, video_name):

    height, width = img_list[0].shape[:2]
    size = (width, height)
    out = cv2.VideoWriter("../videos/" + video_name, cv2.VideoWriter_fourcc(*'mp4v'), 20.0, size, False)

    bar = Bar('Creating Video', max=len(img_list))
